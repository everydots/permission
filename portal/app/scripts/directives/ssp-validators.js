sspApp
  .constant('errorMessages', {
    totalEqualErrorMsg: 'You must allocate 100% of your benefit.',
    rowsNoMoreThanErrorMsg: "You can't add more than 10 beneficiaries.",
    rowsDuplicateErrorMsg: "There are duplicate beneficiary entries. Ensure each individual is only listed once.",
    rowsDOBAndRelationshipErrorMsg: "Date of birth is not valid for the relationship selected.",
    allocationNotEmptyErrorMsg: 'You must allocate more than 0% of your benefit to each beneficiary.',
    relationshipCodeNotEmptyErrorMsg: "Please enter your beneficiary's relationship.",
    relationshipCodeOneEstateErrorMsg: "You can only nominate one Estate.",
    firstNameNotEmptyErrorMsg: "Please enter your beneficiary's first name.",
    firstNameNameFormatErrorMsg: "Please enter a valid beneficiary's first name.",
    lastNameNotEmptyErrorMsg: "Please enter your beneficiary's surname.",
    lastNameNameFormatErrorMsg: "Please enter a valid beneficiary's surname.",
    lastNameLastNameLengthErrorMsg: "This field must contain at least 2 characters.",
    dobNotEmptyErrorMsg: "Please enter a valid date.",
    dobLessErrorMsg: 'You must be born on or after 1st January 1901.',
    dobMoreErrorMsg: 'The date cannot be in the future.',
    dobDateErrorMsg: 'Please enter a valid date.',


    tfnLengthEqualErrorMsg: 'Please complete your TFN.',
    tfnTFNErrorMsg: 'Please input a valid TFN.',
    mobileNumberLengthEqualErrorMsg: 'Please enter a valid phone number.',
    mobileNumberNotEmptyErrorMsg: 'Please input this field.',
    emailNotEmptyErrorMsg: 'Please input this field.',
    emailEmailPatternErrorMsg: 'Email format is incorrect.',

    dateNotEmptyErrorMsg: 'Please enter a valid date.',
    dateLessErrorMsg: 'Date must be after 16 May 2015.',
    dateMoreErrorMsg: 'Date must be before today.',

    businessNameNotEmptyErrorMsg: "Please enter your company's name.",
    employerEmailAddressNotEmptyErrorMsg: "Please enter your company's email address.",
    employerEmailAddressEmailPatternErrorMsg: 'Email format is incorrect.',
    optionsNotEmptyErrorMsg: 'Please select one of the above options.',

    employeeEmailAddressNotEmptyErrorMsg: "Please enter new email address.",
    employeeEmailAddressEmailPatternErrorMsg: 'Email format is incorrect.',

    amountNoLessThanErrorMsg: "The amount must be equal to or greater than the minimum amount.",
    amountNoMoreThanErrorMsg: "The amount must be less than or equal to the maximum amount.",
    amountNotFalseErrorMsg: "Please set a pension amount."
  })
  .factory('validator', ['errorMessages', 'action', '$timeout', function (errorMessages, action) {
    ...
    var normalValidateFns = {
      validateNotEmpty: function (current, expect) {
      ...
      validateEmailPattern: function (current, expect) {
      ...
      validateNameFormat: function (current, expect) {
        return patterns.userNamePattern.test(current) && current.length <= 50;
      },
      validateLastNameLength: function (current, expect) {
  ...
  .factory('action', ["$compile", function ($compile) {
    return {
      ...
      toggleClass: function (propName, isValid, fnNames, currentScope) {
        ...
        if (input.length) {
          if (isValid) {
            container.addClass('no-error');
            container.removeClass('has-error');
          } else {
            container.addClass('has-error');
            container.removeClass('no-error');
          }
        }
        if (fnNames && fnNames.length) {
          fnNames.forEach(function (fnName) {
            ...
            if (!container.find("." + errorName).length) {
              input.after(angular.element('<span></span>').attr('ng-bind', 'validatorsErrors.' + errorName)
                .addClass(errorName));
            }
          });
  ...
  .directive('restrictFloat', ['validator', 'action', function (validator, action) {
    return {
      require: 'ngModel',
      restrict: 'AC',
      scope: {
